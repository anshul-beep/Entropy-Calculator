...
class SampEnGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Button(frame, text="Plot PermEn vs m", command=self.plot_permen_vs_m).grid(row=10, column=0, columnspan=2, pady=10)
    ...
    def calculate_sampen(self):
        ...
        if self.auto_r_value.get():
            r_min = 0.1 * common_std_dev
            r_max = 0.25 * common_std_dev
        ...
        try:
            sampen1, se_sampen1, cp1, A1, B1 = calculate_sampen(time_series, m, optimal_r, delay)
            sampen2, se_sampen2, cp2, A2, B2 = calculate_sampen(ref_time_series, m, optimal_r, delay)
            if np.isnan(sampen1) or np.isnan(sampen2):
                messagebox.showerror("Error", "Failed to calculate Sample Entropy.")
            else:
                self.result_label.config(
                    text=f"SampEn for m={m}, r={optimal_r:.3f}, Delay={delay}\n"
                         f"Dataset 1: SampEn: {sampen1:.3f}, CP: {cp1:.3f}, A: {A1}, B: {B1}\n"
                         f"Dataset 2: SampEn: {sampen2:.3f}, CP: {cp2:.3f}, A: {A2}, B: {B2}"
                )
        except IndexError as e:
            messagebox.showerror("Error", f"An error occurred: {e}")
    ...
    def calculate_permen(self):
        ...
        try:
            permen1 = calculate_permutation_entropy(time_series, m, delay)
            permen2 = calculate_permutation_entropy(ref_time_series, m, delay)
            result_text = f"PermEn for m={m}, Delay={delay}\n" \
                          f"Dataset 1: PermEn: {permen1:.3f}\n" \
                          f"Dataset 2: PermEn: {permen2:.3f}"
