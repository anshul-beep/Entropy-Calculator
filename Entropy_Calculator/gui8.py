...
class SampEnGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Label(frame, text="Number of Windows:").grid(row=6, column=0, sticky=tk.W)
        self.windows_entry = ttk.Entry(frame, textvariable=self.num_windows)
        self.windows_entry.grid(row=6, column=1, sticky=(tk.W, tk.E))

        ttk.Button(frame, text="Load CSV", command=self.load_csv).grid(row=7, column=0, columnspan=2, pady=10)

        ttk.Button(frame, text="Load Reference CSV", command=self.load_ref_csv).grid(row=8, column=0, columnspan=2, pady=10)

        ttk.Button(frame, text="Calculate Sample Entropy", command=self.calculate_sampen).grid(row=9, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Calculate Permutation Entropy", command=self.calculate_permen).grid(row=10, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Plot PermEn vs m", command=self.plot_permen_vs_m).grid(row=11, column=0, columnspan=2, pady=10)
    ...
    def calculate_sampen(self):
        ...
        for i, window in enumerate(windows):
            try:
                sampen, se_sampen, cp, A, B = calculate_sampen(window, m, optimal_r, delay)
                results.append(sampen)
                ref_sampen, ref_se_sampen, ref_cp, ref_A, ref_B = calculate_sampen(ref_windows[i], m, optimal_r, delay)
