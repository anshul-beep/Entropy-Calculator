...
class SampEnGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Button(frame, text="Calculate Sample Entropy", command=self.calculate_sampen).grid(row=7, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Calculate Permutation Entropy", command=self.calculate_permen).grid(row=8, column=0, columnspan=2, pady=10)
    ...
    def calculate_permen(self):
        ...
        try:
            ...
            self.result_label.config(text=f"PermEn for m={m}, Delay={delay}: {permen:.3f}")
        except IndexError as e:
            messagebox.showerror("Error", f"An error occurred: {e}")
