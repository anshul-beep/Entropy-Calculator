...
class SampEnGUI:
    ...
    def plot_permen_vs_m(self):
        ...
        for file_path, time_series in zip(self.file_paths, time_series_list):
            ...
            plt.title(f'Permutation Entropy vs Template Length (m) with Delay={delay} for {file_path}')
            plt.show()

    def plot_sample_entropy(self, results):
        for file_path, sampen_values in results:
            plt.figure(figsize=(10, 6))
            plt.plot(range(1, len(sampen_values) + 1), sampen_values, marker='o', label=f'Sample Entropy - {file_path}')
            ...
            plt.title(f'Sample Entropy vs Window Number - {file_path}')
            plt.legend()
            plt.show()
    ...
    def plot_permutation_entropy(self, results):
        for file_path, permen_values in results:
            plt.figure(figsize=(10, 6))
            plt.plot(range(1, len(permen_values) + 1), permen_values, marker='o', label=f'Permutation Entropy - {file_path}')
            plt.xlabel('Window Number')
            plt.ylabel('Permutation Entropy')
            plt.title(f'Permutation Entropy vs Window Number - {file_path}')
            plt.legend()
            plt.show()
