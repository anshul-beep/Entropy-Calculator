...
class EntropyGUI:
    def __init__(self, root):
        ...
        self.apply_hamming = tk.BooleanVar()  # New variable for Hamming window option
    ...
    def create_widgets(self):
        ...
        self.entropy_menu = ttk.Combobox(frame, textvariable=self.entropy_type, values=["Sample Entropy", "Permutation Entropy", "Approximate Entropy", "Spectral Entropy"])
        ...
        # Checkbox for applying Hamming window
        self.hamming_check = ttk.Checkbutton(frame, text="Apply Hamming Window", variable=self.apply_hamming)
        self.hamming_check.grid(row=16, column=0, columnspan=2)

        # Parameters for spectral entropy
        self.fft_length_label = ttk.Label(frame, text="FFT Length (N):")
        self.fft_length_label.grid(row=17, column=0, sticky=tk.W)
        self.fft_length_entry = ttk.Entry(frame, textvariable=self.fft_length)
        self.fft_length_entry.grid(row=17, column=1, sticky=(tk.W, tk.E))

        self.freq_min_label = ttk.Label(frame, text="Min Frequency:")
        self.freq_min_label.grid(row=18, column=0, sticky=tk.W)
        self.freq_min_entry = ttk.Entry(frame, textvariable=self.freq_min)
        self.freq_min_entry.grid(row=18, column=1, sticky=(tk.W, tk.E))

        self.freq_max_label = ttk.Label(frame, text="Max Frequency:")
        self.freq_max_label.grid(row=19, column=0, sticky=tk.W)
        self.freq_max_entry = ttk.Entry(frame, textvariable=self.freq_max)
        self.freq_max_entry.grid(row=19, column=1, sticky=(tk.W, tk.E))

        ttk.Button(frame, text="Load Files", command=self.load_files).grid(row=20, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Add Files", command=self.add_files).grid(row=21, column=0, columnspan=2, pady=10)

        ttk.Button(frame, text="Calculate Entropy", command=self.calculate_entropy).grid(row=22, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Plot Entropy", command=self.plot_entropy).grid(row=23, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Download Results as CSV", command=self.download_results).grid(row=24, column=0, columnspan=2, pady=10)

        self.result_label = ttk.Label(frame, text="")
        self.result_label.grid(row=25, column=0, columnspan=2, pady=10)
