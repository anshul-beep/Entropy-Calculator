...
class EntropyGUI:
    ...
    def create_widgets(self):
        ...
        self.entropy_menu = ttk.Combobox(frame, textvariable=self.entropy_type, values=["Sample Entropy", "Permutation Entropy", "Approximate Entropy"])
        ...
        ttk.Button(frame, text="Load Files", command=self.load_files).grid(row=17, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Add Files", command=self.add_files).grid(row=18, column=0, columnspan=2, pady=10)

        ttk.Button(frame, text="Calculate Entropy", command=self.calculate_entropy).grid(row=19, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Plot Entropy", command=self.plot_entropy).grid(row=20, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Download Results as CSV", command=self.download_results).grid(row=21, column=0, columnspan=2, pady=10)

        self.result_label = ttk.Label(frame, text="")
        self.result_label.grid(row=22, column=0, columnspan=2, pady=10)
